  ...
  describe('redeem', () => {
    it('should create redeem transaction for valid attestation', async () => {
      ...
      mockClient.getObject
        ...
        .mockResolvedValueOnce(mockSuiObject('0xwormhole::wormhole_transceiver::State', {})); // getWormholePackageId
    ...
    it('should throw error for unsupported attestation type', async () => {
      ...
      await expect(txGenerator.next()).rejects.toThrow('Unsupported attestation type: UnsupportedType');
    });
    ...
    it('should throw error for non-SUI tokens', async () => {
      ...
      await expect(txGenerator.next()).rejects.toThrow('Redeem not yet implemented for token: 0xabc::custom::TOKEN');
    });
    ...
    it('should throw error when wormhole transceiver not configured', async () => {
      ...
      mockClient.getObject.mockResolvedValueOnce(mockSuiObject('0xpackage::ntt::State', {}));

      const attestation = mockAttestation('Ethereum');
      
      const txGenerator = suiNttWithoutTransceiver.redeem([attestation]);
      await expect(txGenerator.next()).rejects.toThrow('Wormhole transceiver not configured');
    });
  });
});