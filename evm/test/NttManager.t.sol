...
contract TestNttManager is Test, IRateLimiterEvents {
    ...
    function setUp() public {
        ...
        nttManager.setPeer(
            chainId2, toWormholeFormat(address(nttManagerOther)), 9, type(uint64).max
        );

        nttManagerOther.setPeer(chainId, toWormholeFormat(address(nttManager)), 9, type(uint64).max);
    ...
    function test_upgradeNttManager() public {
        ...
        */
        ...
        // // Setup created the two manangers, each with one transceiver pointed at each other. We just need to add one more to each.
        // DummyTransceiver transceiver2 = new DummyTransceiver(chainId, address(router));
        // nttManager.setTransceiver(address(transceiver2));
        // nttManager.enableSendTransceiver(chainId2, address(transceiver2));
        // nttManager.enableRecvTransceiver(chainId2, address(transceiver2));

        // DummyTransceiver transceiverOther2 = new DummyTransceiver(chainId2, address(routerOther));
        // nttManagerOther.setTransceiver(address(transceiverOther2));
        // nttManagerOther.enableSendTransceiver(chainId, address(transceiverOther2));
        // nttManagerOther.enableRecvTransceiver(chainId, address(transceiverOther2));

        // token.mintDummy(address(user_A), 5 * 10 ** token.decimals());

        // // Test outgoing transfers complete successfully with rate limit disabled
        // vm.startPrank(user_A);
        // token.approve(address(nttManager), 3 * 10 ** token.decimals());

        // vm.recordLogs();

        // uint64 seq =
        //     nttManager.transfer(1 * 10 ** token.decimals(), chainId2, toWormholeFormat(user_B));
        // require(seq == 0, "seq is not zero");

        // // Attest the message on first transceiver on the other side.
        // {
        //     DummyTransceiver.Message[] memory routerMsgs = transceiver.getMessages();
        //     assert(1 == routerMsgs.length);
        //     DummyTransceiver.Message memory rmsg = routerMsgs[0];
        //     transceiverOther.receiveMessage(rmsg);
        //     checkAttestation(nttManagerOther, 0, rmsg);

        //     // Attest the message on second transceiver on the other side.
        //     routerMsgs = transceiver2.getMessages();
        //     assert(1 == routerMsgs.length);
        //     rmsg = routerMsgs[0];
        //     transceiverOther2.receiveMessage(rmsg);
        //     checkAttestation(nttManagerOther, 1, rmsg);
        // }

        // // Execute the message on the other side.
        // {
        //     // Get the execution events from the logs.
        //     Vm.Log[] memory logEvents = vm.getRecordedLogs();
        //     (, bytes memory payload) =
        //         getExecutionSent(logEvents, chainId, address(nttManager), seq);

        //     nttManagerOther.executeMsg(
        //         chainId, UniversalAddressLibrary.fromAddress(address(nttManager)), seq, payload
        //     );
        // }
    }
